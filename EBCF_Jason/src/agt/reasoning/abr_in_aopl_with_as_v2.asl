...
answer([cq(Cq_name,Pred)[as(AS)]]):- (strict_inference(Pred) | defeasible_inference(Pred)).// & .print("Critical Question ", Pred, " answered!"). 
answer([cq(Cq_name,Pred)[as(AS)]|T]):- (strict_inference(Pred) | defeasible_inference(Pred))// & .print("Critical Question ", Pred, " answered!") 
& answer(T). 
answer([]).
...
strict_inference(Result,Arg,Goal) :-  not .list(Goal) & Goal & .concat(Arg,[Goal],Result). // necessary for predicates derived from (Jason's Prolog-like) inference rules
