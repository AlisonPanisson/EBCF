...
/* ######## Justify ######### */
@kqmlReceivedJustify
+!kqml_received(Sender, justify, justify(Content,D)[as(AS),Unifier], MsgId): .list(Content) & decodification(Content,AS,Unifier,Arg)
   <- 	//.print("Received justify message with ", Content, " based on ", AS, " and ", Unifier);
        //!add_all_kqml_received(Sender,Content);        
        !responseJustify(Sender,Arg,D). 
        
/* ######## Justify ######### */
@kqmlReceivedJustify2
