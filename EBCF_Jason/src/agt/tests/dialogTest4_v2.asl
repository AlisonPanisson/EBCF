...
{ include("performatives/performatives3_v2.asl") }
{ include("enthymemes/enthymemes.asl") }
...
isSubject(Content):- isSubject(Content)[dialogue(_)].
...
+!responseJustify(Sender,[defeasible_rule(C,S)[as(AS)]|Premises],Dialogue): isSubject(access(Name,Info)) & contrary(access(Name,Info),C) & role(Name,Role) & .member(satisfies(Name,role(Role)),Premises) 
																				...
																				& .my_name(MyName) & .print("Received Message:  JUSTIFY(",Sender, ",", MyName, "," , [defeasible_rule(C,S)[as(AS)],Premises],Dialogue,")")
...
+!respondQuestion(Sender,Content,D): has_argument_against(Content,Justification) & enthymemetisation(Justification,JE,AS,Unifier) & .my_name(MyName) & .print("Received Message: QUESTION(",Sender, ",", MyName, "," , Content, ")")
<- 	    allAgents(Ag)[artifact_id(A)];
		.term2string(AgT,Ag);
		+isSubject(Content)[dialogue(D)];
	     performatives.send(AgT,justify,justify(JE,D)[as(AS),Unifier]).

+!respondQuestion(Sender,Content,D) <- .my_name(MyName); .print("Received Message: QUESTION(",Sender, ",", MyName, "," , Content, ")").


/* ACCEPT */
+!receiveAccept(Sender,Content[dialogue(Dialogue),source(Source)]): isSubject(Content,Dialogue) & .my_name(MyName) & .print("Received Message: ACCEPT(",Sender, ",", MyName, "," , Content, ")")
	<- !closedialogue(Dialogue).
	
+!receiveAccept(Sender,Content[dialogue(Dialogue),source(Source)]): not(isSubject(Content,Dialogue)) & .my_name(MyName) & .print("Received Message: ACCEPT(",Sender, ",", MyName, "," , Content, ")").
...
+!check_subject(Sender,Dialogue): isSubject(Content) & consideringCS(Sender)[dialogue(Dialogue)] & has_argument_against(Content,Arg) & enthymemetisation(Arg,JE,AS,Unifier)
	<- 	-consideringCS(Sender)[dialogue(Dialogue)];
		performatives.send(Sender,justify,justify(JE,Dialogue)[as(AS),Unifier]).
